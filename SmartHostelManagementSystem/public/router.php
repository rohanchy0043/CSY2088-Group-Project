...
$wardenRoutes = [
    ...
    '/warden/room-create' => ['GET', 'roomCreate'],
    '/warden/room-store' => ['POST', 'roomStore'],
...
if (preg_match('#^/warden/(notifications|profile|settings)$#', $path, $matches) && $method === 'GET') {
...
if (preg_match('#^/warden/room-edit/(\d+)$#', $path, $matches)) {
    if ($method === 'GET') {
        WardenController::roomEdit((int) $matches[1]);
    } elseif ($method === 'POST') {
        WardenController::roomUpdate((int) $matches[1]);
    } else {
        abort(405, 'Method Not Allowed');
    }
    exit;
}
if (preg_match('#^/warden/fee-mark-paid/(\d+)$#', $path, $matches) && $method === 'POST') {
    WardenController::feeMarkPaid((int) $matches[1]);
    exit;
}
if (preg_match('#^/warden/room-delete/(\d+)$#', $path, $matches) && $method === 'POST') {
    WardenController::roomDelete((int) $matches[1]);
    exit;
}
